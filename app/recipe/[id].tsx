...
import { ChevronLeft, Heart, ShoppingCart, Tag } from "lucide-react-native";
...
export default function RecipeDetail() {
  ...
  const { toggleFavorite, isFavorite, fridgeItems, addToGrocery, groceryList } = useApp();
    ...
    return (
      <View
        style={{
          ...
          backgroundColor: isDark ? "#0d0d0d" : "#FFF7ED",
        }}
      >
          ...
          {fridgeItems.length > 0 && (
            <View style={[s.summary, { backgroundColor: colors.bgCard, borderColor: colors.border }]}>
              <Text style={[s.summaryText, { color: colors.textMuted }]}>
                You have{" "}
                <Text style={[s.summaryBold, { color: colors.accent }]}>
              ...
              <View style={[s.summaryBar, { backgroundColor: colors.border }]}>
                <View
                  style={[
                    s.summaryFill,
                    {
                      width: `${ingredients.length ? (have / ingredients.length) * 100 : 0}%` as any,
              ...
              <View key={i} style={s.ingRow}>
                ...
                {!ing.inFridge && (
                  ...
                  >
                    <ShoppingCart
                      size={11}
                      color={inList ? "#22c55e" : colors.accent}
                      strokeWidth={2}
                    />
                    <Text
                      style={[

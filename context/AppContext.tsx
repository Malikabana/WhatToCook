...
type ContextType = {
  ...
  clearGrocery: () => void;
  recentlyViewed: Meal[];
  addRecentlyViewed: (meal: Meal) => void;
};
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [recentlyViewed, setRecentlyViewed] = useState<Meal[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [f, fr, gr, rv] = await Promise.all([
  ...
  const addRecentlyViewed = (meal: Meal) => {
    const next = [meal, ...recentlyViewed.filter((m) => m.idMeal !== meal.idMeal)].slice(0, 10);
